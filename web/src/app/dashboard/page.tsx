...
export default function Home() {
  ...
  const [agentInitial, setAgentInitial] = useState<'discovery' | 'analyst' | 'sourcing' | 'negotiator'>('discovery');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {dataSource === 'mock' && (
          <div className="mb-4 px-4 py-3 bg-amber-50 border-2 border-amber-300 rounded-lg text-sm text-amber-800 flex items-center gap-2">
            <svg className="w-5 h-5 text-amber-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
            <div>
              <span className="font-semibold">DEMO</span> — Données fictives. Les actions (sourcing, export, IA) sont désactivées.
            ...
            <div className="flex items-center gap-8">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-emerald-600">
                  {formatCurrency(Math.round(summary.totalPotentialValue))}
                </div>
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-gray-400">
                  Score ≥ {summary.criteria.minScore} | Valeur ≥ {formatCurrency(summary.criteria.minValue)}
                </div>
              </div>
            </div>
          </div>
        ...
        {!isLoading && (
          <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
            <div className="flex flex-wrap items-center gap-4">
              ...
              {/* Export CSV button — disabled in DEMO mode */}
              <button
                onClick={handleExportCSV}
                disabled={isExporting || dataSource === 'mock'}
                title={dataSource === 'mock' ? 'Export désactivé en mode démo' : undefined}
                className="ml-auto flex items-center gap-2 px-4 py-1.5 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              {selectedOpportunity ? (
                <div className="space-y-4">
                  <OpportunityDetail
                    ...
                    onStartSourcing={dataSource === 'api' ? () => {
                      setAgentInitial('sourcing');
                      setShowAgentChat(true);
                    } : undefined}
                    isDemo={dataSource === 'mock'}
                  />

                  {/* Agent Chat Button — disabled in DEMO mode */}
                  <button
                    onClick={() => { if (dataSource === 'mock') return; setAgentInitial('discovery'); setShowAgentChat(true); }}
                    disabled={dataSource === 'mock'}
                    className={`w-full flex items-center justify-center gap-3 px-6 py-4 rounded-xl shadow-lg transition-all group ${
                      dataSource === 'mock'
